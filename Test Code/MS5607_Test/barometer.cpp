...
// Primes MS56XX to orgainze pressure data for reading
...
uint32_t readRawMS56XX(void) {
  ...
  //if(Wire.available() == 0)
  //Wire.requestFrom(BARO_ADDRESS-2, 3);
  
  return (((uint32_t)Wire.read() << 16) | ((uint32_t)Wire.read() << 8) | (uint32_t)Wire.read());
}

/* Reads MS56XX temperature data into MS56XXdata struct
 * Assumptions:
...
/* Reads MS56XX pressure data into MS56XXdata struct
 * Assumptions:
 *    MS56XX primed for pressure
 *    10 ms delay after priming
*/
...
void calcAltitudeMS56XX(BAROMETER_packet *data) {
